...
public class DnsPrimitivesTests
{
    ...
    public void TryWriteQName_BufferTooShort_Fails()
    {
        ...
        {
            Assert.False(DnsPrimitives.TryWriteQName(buffer.AsSpan(0, i), name, out int written));
        }
    }
    ...
    public void TryReadQName_TruncatedData_Fails()
    {
        ...
        {
            Assert.False(DnsPrimitives.TryReadQName(data.Slice(0, i), 0, out string? actual, out int bytesRead));
        }
    }
